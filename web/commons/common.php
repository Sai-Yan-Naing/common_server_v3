...
function IpAndDomainRestriction($user)
{
	...
	for ($i = 0; $i < count($shell); $i++)
	{
		...
		elseif (strpos(strtolower(preg_replace('/\s+/', '', $shell[$i])), 'subnetmask') !== false)
		{
...
function siteBinding($site_name, $do, $ip = '*', $domain)
{
	// die($site_name.$do.$ip.$domain);
	shell_exec("%systemroot%\system32\inetsrv\appcmd.exe set site /site.name:$site_name /".$do."bindings.[protocol='http',bindingInformation='".$ip.':80:'.$domain."']");
}
...
function addBassman($user, $bassman_setting)
{
	$bassman_file = getFile($user.'/bassman/Bassman.setting');
	if (file_exists($bassman_file))
	{
		unlink($bassman_file);
	}
	...
	foreach (json_decode($bassman_setting) as $bass_key => $bass_value)
	{
