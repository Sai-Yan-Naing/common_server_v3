<div class="shadow-lg mb-3 bg-white rounded d-flex font-weight-bold" style="letter-spacing: 3px; font-size:12px">
        ...
        <a href="/admin/share/server?setting=security&tab=ssl&act=index&webid=<?=$webid?>" class="subtitle pt-4 pb-4 <?=($setting=='security')?"active":"text-dark"?>" onclick="loading()">
            <img src="<?= call_ass() ?>img/subtitle/<?=($setting=='security')?"security1":"security"?>.png" alt="site.png">
            <br><br>
            <span class="pl-3 pr-3">サイトセキュリティー</span>
        </a>
        <a href="/admin/share/server?setting=database&tab=mysql&act=index&webid=<?=$webid?>" class="subtitle pt-4 pb-4 <?=($setting=='database')?"active":"text-dark"?>" onclick="loading()">
            <img src="<?= call_ass() ?>img/subtitle/<?=($setting=='database')?"database1":"database"?>.png" alt="site.png">
            <br><br>
            <span class="pl-3 pr-3">データベース</span>
        </a>
        <a href="/admin/share/server?setting=ftp&tab=tab&act=index&webid=<?=$webid?>" class="subtitle pt-4 pb-4 <?=($setting=='ftp')?"active":"text-dark"?>" onclick="loading()">
            <img src="<?= call_ass() ?>img/subtitle/<?=($setting=='ftp')?"ftp1":"ftp"?>.png" alt="site.png">
            <br><br>
            <span class="pl-3 pr-3">FTP</span>
        </a>
        <a href="/admin/share/server?setting=filemanager&tab=tab&act=index&webid=<?=$webid?>" class="subtitle pt-4 pb-4 <?=($setting=='filemanager')?"active":"text-dark"?>" onclick="loading()">
